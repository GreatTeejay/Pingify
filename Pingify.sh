...
#  engine source, so a single file is everything either server needs.
...
GO_MIN_MINOR=19          # the engine needs Go 1.19 or newer
...
item() {
    if [ -n "${3:-}" ]; then
        printf '    %s%s%s  %s%s%s%s\n' \
            "$C_CYN$C_B" "$1" "$C_OFF" "$(pad_to "$2" 26)" "$C_DIM" "$3" "$C_OFF"
    else
        printf '    %s%s%s  %s\n' "$C_CYN$C_B" "$1" "$C_OFF" "$2"
    fi
}
...
banner() {
    ...
    printf '  %s%s  Iran %s Kharej tunnel  %s  v%s%s\n\n' \
        "$C_DIM" "$(repeat "$BX_H" 4)" "$BX_ARR" "$BX_DOT" "$PINGIFY_VERSION" "$C_OFF"
}
...
ask() {
    ...
    if [ -n "$__def" ]; then
        read -rp "  ${C_CYN}${BX_ARR}${C_OFF} ${__prompt} ${C_DIM}[${__def}]${C_OFF} " __in
        [ -z "$__in" ] && __in="$__def"
    else
        read -rp "  ${C_CYN}${BX_ARR}${C_OFF} ${__prompt} " __in
    fi
...
confirm() {
    local reply=""
    read -rp "  ${C_YEL}?${C_OFF} $1 ${C_DIM}[y/N]${C_OFF} " reply
