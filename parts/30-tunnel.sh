...
mode_of() {
    case $1 in
    tcp | ws | wss | utls | fallback | kcp) printf 'forward' ;;
    *) printf 'tun' ;;
    esac
}
cfg_mode() {
...
forwarding_transport() { [ "$(mode_of "$1")" = forward ]; }
...
# both.
binds_here() { this_side_waits || case $T_TRANSPORT in awg | grefou) true ;; *) false ;; esac; }
...
cfg_render() {
    ...
    *) kv port "$T_PORT" ;;
    ...
    if [ "$mode" = forward ] && [ -n "$T_BACKUPS" ]; then
        printf '\n[failover]\n'
        # shellcheck disable=SC2086
        kv backups "[$(fwd_toml_list $T_BACKUPS)]"
        kv enabled "${T_FO_ENABLED:-true}"
        kv prefer "$(q "${T_FO_PREFER:-order}")"
        kv switch_after_sec "${T_FO_SWITCH:-25}"
        kv return_after_sec "${T_FO_RETURN:-120}"
...
tunnel_create() {
    ...
    # The ports live in the file so that they travel in the token, and in
    # the forwards state on IRAN so that the Ports screen and the clash
    # check know them. A private link's go into the firewall as well; a
    # forward tunnel's are the core's own sockets.
    if [ "$(toml_get "$f" tunnel side)" = iran ] && [ ! -s "$(fwd_file "$name")" ]; then
        local ports
        ...
        if [ -n "$ports" ]; then
            ...
            elif [ "$(toml_get "$f" tunnel mode)" = tun ] && have iptables; then
    ...
    awg)
        ...
        ;;
    grefou)
        # The kernel device, its FOU listener, the offload setting and the
        # clamp - all of it before the core, which only watches them.
        if ! grefou_up "$name"; then
            rm -f "$f"
            return 1
        fi
        ok "the kernel link $(grefou_iface "$name") is up"
        ;;
    rawtcp)
    ...
    # The file is written and the tunnel exists; whether it started is a
    # separate question. The caller prints the token either way, because
    # without it the other server cannot be built at all.
    svc_do enable "$name" || {
        warn "it is on disk and will start again by itself; the token below still stands"
        return 0
    }
}

# show_setup_token NAME - the token again, for a pair that was half built
# or a screen that was closed. It is the same line the wizard printed.
show_setup_token() {
    local name=$1
    cfg_load "$name" || { fail "there is no tunnel called $name here"; return 1; }
    banner
    head2 "Setup token: $name"
    dim "paste this into the other server:  New tunnel ${BX_ARR} Paste a token"
    blank
    rule
    printf '%s
...
cfg_setup_token() {
    local body sum
    body=$(printf 'p8|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s' \
        ...
        "$(tok_enc "$T_BACKUPS")" "$T_FO_SWITCH" "$T_FO_RETURN" "$T_FO_ENABLED" "$T_FO_PREFER")
...
setup_token_read() {
    ...
    local backups fo_switch fo_return fo_enabled fo_prefer need
    SETUP_TOKEN_ERROR=
    line=${1//[[:space:]]/}
    case $line in
    PFY3.*) ;;
    ...
    *) setup_token_bad "that is not a Pingify setup token - it starts with PFY3."; return 1 ;;
    ...
    # p7 is p6 with the failover backups on the end, p8 has the switch and
    # the preference too; an older token simply has none.
    case $raw in
    p6\|*) need=36 ;;
    p7\|*) need=39 ;;
    p8\|*) need=41 ;;
    *) setup_token_bad "the token will not decode - copy the whole line"; return 1 ;;
    esac
    body=${raw%|*}
    sum=${raw##*|}
    ...
    [ "$fields" = "$need" ] || { setup_token_bad "the token is incomplete"; return 1; }
    IFS='|' read -r v name tr mode dials kh ir port path conns tok preset lg health \
        oct tunif mtu fwd aif aport ikey ipub kkey kpub jc jmin jmax s1 s2 h1 h2 h3 h4 fec queue ver \
        backups fo_switch fo_return fo_enabled fo_prefer <<<"$body"
    ...
    T_BACKUPS=$(tok_dec "$backups") || T_BACKUPS=
    T_FO_SWITCH=$fo_switch T_FO_RETURN=$fo_return
    T_FO_ENABLED=$fo_enabled T_FO_PREFER=$fo_prefer
    TOKEN_VERSION=$ver
    setup_token_check
}
...
' "$body" >"$tmp"
    ...
    # The name becomes a file name and a unit name here; the token is
    # checksummed, not signed.
    v_name "$T_NAME" >/dev/null 2>&1 || { setup_token_bad "the token names its tunnel with characters a name cannot have"; return 1; }
...
ask_side() {
    ...
    choice 1 "IRAN" "users connect here, and the ports live here"
...
ask_addresses() {
    ...
    local peers peer_def=
    peers=$(known_peers)
    if [ -n "$peers" ]; then
        dim "the tunnels here already reach:"
        printf '%s\n' "$peers" | while read -r a names; do
            [ -n "$a" ] && printf '%s\n' "$names" | tr ',' '\n' | dim_wrap "$(pad_to "$(addr_tint "$a")" 18)${BX_ARR} " ", "
        done
        [ "$(printf '%s\n' "$peers" | grep -c .)" = 1 ] && peer_def=${peers%% *}
        blank
    fi
    case $T_TRANSPORT in
...
# known_peers - "address name,name" for every other server the tunnels here
# reach, so a second tunnel to the same server is one enter rather than an
# address typed again.
known_peers() {
    local f side a n
    cfg_files | while read -r f; do
        side=$(toml_get "$f" tunnel side)
        if [ "$side" = iran ]; then a=$(toml_get "$f" transport kharej); else a=$(toml_get "$f" transport iran); fi
        [ -n "$a" ] || continue
        n=$(cfg_name "$f")
        printf '%s %s\n' "$a" "$n"
    done | awk '{ names[$1] = (names[$1] == "" ? $2 : names[$1] "," $2) } END { for (a in names) print a, names[a] }' | sort
}
...
# failover
#
# The transports a forward tunnel moves to by itself when the one it runs on
# stops carrying, and moves back from when that one recovers. The core does
# the moving - see docs/failover.md - so all that is asked here is which, in
# what order. The ports are chosen, not asked: KCP is UDP and shares the
# tunnel's number, the rest take the next free ones above it.
# ---------------------------------------------------------------------------

# The forwarding transports in the order of the transport menu, so a backup
# has the number it has there.
BACKUP_KINDS="tcp ws wss utls fallback kcp"

v_backups() {
    local n seen=" " k
    [ -n "${1//[, ]/}" ] || return 0
    for n in ${1//,/ }; do
        case $n in [1-6]) ;; *) echo "numbers from the list, like 6,4"; return 1 ;; esac
        k=$(printf '%s\n' $BACKUP_KINDS | sed -n "${n}p")
        [ "$k" = "$T_TRANSPORT" ] && { echo "$n is the transport the tunnel already runs on"; return 1; }
        case $seen in *" $n "*) echo "$n is in the list twice"; return 1 ;; esac
        seen="$seen$n "
    done
    return 0
}

# backup_port KIND - a port for a backup: free of the tunnel's own port and of
...
# backups_panel puts the same in a panel, one member to a line, so that a
# list of three is not cut off at the edge of the box.
backups_panel() {
    local b i=0 tail=
    [ -n "$T_BACKUPS" ] || { panel_field "Failover" "none"; return 0; }
    [ "$T_FO_ENABLED" = false ] && tail=" (off - the list is kept)"
    for b in $T_BACKUPS; do
        i=$((i + 1))
        if [ "$i" = 1 ]; then
...
backups_menu() {
    ...
    for k in $BACKUP_KINDS; do
        i=$((i + 1))
        [ "$k" = "$T_TRANSPORT" ] && continue
        choice "$i" "$(transport_label "$k")"
    done
}
...
ask_backups() {
    ...
    dim "The move is automatic, and so is the move back once it recovers."
    blank
...
new_tunnel() {
    ...
    if [ "$T_TRANSPORT" = awg ]; then
        ...
        # Installed here, after the last question, so that q at any of
        # them leaves the server as it was found.
        awg_install || { pause; wiz_end; return 1; }
    ...
    *) fail "that token does not say which server made it"; pause; return 1 ;;
    ...
    if [ -e "$(cfg_file "$T_NAME")" ]; then
        ...
        if confirm "replace it?"; then
            tunnel_remove "$T_NAME" >/dev/null 2>&1
        else
            clash=1
        fi
    fi
    ...
    if [ "$T_TRANSPORT" = awg ]; then
        ...
        for b in $T_BACKUPS; do
            ...
            if [ -n "$own" ]; then
                ...
                clash=1
            elif ! port_free "$bport" "$bfam"; then
                fail "something here already listens on ${bport}/$bfam, which the $(transport_label "${b%%:*}") backup needs"
                clash=1
            fi
        done
    fi
    ...
    # A token made on KHAREJ first carries no ports, and IRAN is where they
    # live: the one question the other side could not answer.
    if [ "$T_SIDE" = iran ] && [ -z "${T_FORWARDS:-}" ]; then
        ask_forwards || { pause; return 1; }
    fi
