...
install_self() {
    ...
    # Written whenever the script changed, not only when its version did: a
    # unit that gained a line between two builds of the same version was
    # otherwise never rewritten, and the tunnel it was for could not start
    # after a reboot.
    stamp=$STATE_DIR/script.version
    if [ "$(cat "$stamp" 2>/dev/null)" != "$PINGIFY_VERSION" ] || [ "$SCRIPT_CHANGED" = 1 ]; then
