...
run_health_check() {
    ...
    while IFS= read -r n; do
        ...
        # Stopped on purpose, from the menu: not a tunnel that fell over.
        [ -f "$STATE_DIR/$n.stopped" ] && continue
        f=$(cfg_file "$n")

        if ! systemctl is-active --quiet "pingify@$n"; then
            echo "pingify: $n is not running, starting it"
            # A unit that spent its start limit refuses every later start
            # until the count is cleared, and the watchdog is the only
            # thing that would ever clear it.
            systemctl reset-failed "pingify@$n" >/dev/null 2>&1
        ...
        # and the health log would say nothing but that. Heard is written
        # down when it happens: the status call that fails on a wedged core
        # reports no counters at all, and that is the one case a restart
        # is for.
        [ "${ST_INB:-0}" = 0 ] || : >"$STATE_DIR/$n.heard"
        [ -f "$STATE_DIR/$n.heard" ] || strike=0
        if [ "$strike" = 0 ]; then
            echo 0 >"$STATE_DIR/$n.fail"
            continue
        fi
...
# enable_watchdog [quiet] - quiet is the wizard's call after every tunnel,
# and it respects a watchdog the user turned off; only the menu turns it
# back on.
enable_watchdog() {
    if [ "${1:-}" = quiet ] && [ -f "$STATE_DIR/watchdog.off" ]; then
        return 0
    fi
    rm -f "$STATE_DIR/watchdog.off"
...
disable_watchdog() {
    ...
    mkdir -p "$STATE_DIR" 2>/dev/null && : >"$STATE_DIR/watchdog.off"
    ok "watchdog disabled - it stays off until you enable it here"
}
...
health_check() {
    ...
    *)
    ...
    if [ "$st" = active ] && tun_stats "$name"; then
        ...
        # Three states, not two: heard and up, heard and since lost, never
        # heard. The middle one used to be reported as the last, with advice
        # about firewalls for a tunnel that had run for hours.
        [ "$heard" = false ] && [ "${ST_INB:-0}" != 0 ] && heard=lost
        case $heard in
        lost)
            chk_add bad peer "the other server was heard from, and has been silent for $(human_secs "${ST_FAR_SEEN%%.*}")"                 "the core keeps trying by itself, and the watchdog restarts it after three strikes"                 "if it stays silent, run this check on $CK_FAR too"
            ;;
        true)
            if [ "${ST_INB:-0}" = 0 ]; then
        ...
        *)
            ...
            icmp)
                chk_add bad peer "the other server has never been seen" \
                    ...
                    "if nothing arrives at all, the route drops ICMP - try Chrome TLS MUX or Fake TCP"
                ;;
            awg)
            ...
            *)
                chk_add bad peer "the other server has never been seen" \
                    ...
                    "most Iranian lines stop inbound UDP after a few packets; ICMP or Fake TCP is the usual answer"
                ;;
            esac
            ;;
        esac

        if [ "$CK_TRANSPORT" = grefou ]; then
            local gdev gul ggro
            gdev=$(grefou_iface "$name")
            if [ -z "$gdev" ] || ! ip link show "$gdev" >/dev/null 2>&1; then
                chk_add bad grefou "the kernel link ${gdev:-for this tunnel} is not there" \
                    "it is made before the core starts:  systemctl restart pingify@$name"
            else
                chk_add ok grefou "the kernel carries this tunnel on $gdev - the core only watches it"
            fi
            if ! ip fou show 2>/dev/null | grep -q "port $CK_PORT "; then
                chk_add bad fou "nothing here unwraps udp/$CK_PORT, so the far end's packets are dropped" \
                    "systemctl restart pingify@$name" "the module it needs is fou"
            fi
            gul=$(grefou_underlay "$(peer_public "$name")")
            ggro=$(gro_state "$gul")
            if [ -n "$gul" ] && [ -n "$ggro" ] && [ "$ggro" != off ]; then
                chk_add bad gro "generic receive offload is on for $gul, and this transport carries nothing while it is" \
                    "ethtool -K $gul gro off" \
                    "restarting the tunnel sets it too; it goes back when the tunnel is deleted"
        ...
        if [ "$CK_TRANSPORT" = awg ]; then
            ...
            if [ -z "$iface" ] || ! ip link show "$iface" >/dev/null 2>&1; then
                ...
                chk_add bad awg "AmneziaWG on $iface has never completed a handshake" \
        ...
        if have curl; then
            chk_add bad status "no answer on 127.0.0.1:$(status_port "$name")" \
                "systemctl status pingify@$name" "if the tunnel is otherwise well, restart it: systemctl restart pingify@$name"
        else
    ...
    if [ "$st" = active ] && [ "${ST_UP:-}" = true ] && [ -n "${ST_UPTIME:-}" ]; then
        ...
        *)
            ...
            *) [ "$(toml_get "$CK_FILE" tuning fec)" -gt 0 ] 2>/dev/null ||
            ...
            # What matters is the share of packets lost, not the count: a
            # tunnel that carried a million in a minute and lost twelve
            # thousand lost one in a hundred, and the count alone called
            # that a problem the minute after any big transfer. The share
            # decides; the rate is there for scale.
            local share=0.00 arrived
            arrived=$(( ${ST_TODEV:-0} + ${ST_LOST:-0} ))
            [ "$arrived" -gt 0 ] &&
                share=$(awk -v l="${ST_LOST:-0}" -v a="$arrived" 'BEGIN { printf "%.2f", l * 100 / a }')
            if [ "${share%%.*}" -ge 2 ]; then
                chk_add bad loss "the path is losing $share% of what it carries, $lpm packets a minute" \
                    "run Find the MTU; a large mtu loses big packets" \
                    ${fec_advice:+"$fec_advice"} "if the mtu is right, the path is congested"
            elif [ "$share" != "0.00" ] && [ "${lpm%%.*}" -ge 5 ]; then
                chk_add warn loss "the path is losing $share% of what it carries, $lpm packets a minute" \
                    "run Find the MTU: a slightly large mtu does this" ${fec_advice:+"$fec_advice"}
            else
                chk_add ok loss "loss is $share% of what it carries, $lpm a minute, ${ST_LATE:-0} arrived late"
            fi
            ;;
        esac
    fi
    ...
    if [ "$CK_SIDE" = iran ]; then
        ...
        if [ -z "$spec" ]; then
            ...
            while read -r proto lo hi rhost rport; do
                ...
                [ "$rhost" = - ] && rhost=$CK_PEER
                total=$((total + 1))
                # No chain at all is every rule missing, not nothing to
                # check; and 80 is not a match for 8000:8010.
                if [ "$CK_MODE" = tun ] && have iptables &&
                    ! printf '%s
' "${rules:-}" | grep -qE -- "--dport $lo( |:)"; then
...
health_menu() {
    ...
    while :; do
        ...
        5) blank
            head2 "Health log"
            dim "what the watchdog found on its rounds, newest at the bottom"
            blank
            # With the time. Without it the log is a list of complaints
            # with no way to tell last week's from this minute's.
            if ! journalctl -u pingify-health.service -n 40 --no-pager                 -o short-iso --no-hostname 2>/dev/null |
                sed -E 's/^([0-9-]+)T([0-9:]+)[^ ]* [^:]*: /     /' | grep -v "^  *--" | grep .; then
                dim "nothing yet - the watchdog says something only when it finds something"
            fi
            pause ;;
        6) blank
            confirm "restart every tunnel on this server? each drops for a second" n || continue
            for n in $(cfg_list); do svc_do restart "$n"; done; pause ;;
        0 | '') return 0 ;;
        *) blank; warn "there is nothing on $c"; sleep 1 ;;
        esac
    done
