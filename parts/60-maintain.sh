...
update_pingify() {
    ...
    if [ "$rc" != 0 ]; then
        ...
        dim "on an Iranian server this often fails; fetch it on the KHAREJ server and copy it across"
        pause; return 1
    fi
...
remove_menu() {
    ...
    1) blank
        ...
        # Stopped and not merely killed: with the binary gone, systemd and
        # the watchdog would otherwise retry every tunnel for ever.
        for n in $(cfg_list); do svc_do stop "$n" >/dev/null 2>&1; done
        rm -f "$CORE_BIN"
        rm -rf "$SRC_DIR"
        ok "core removed; the tunnels stay stopped until the next run builds it again"
        pause ;;
    2) blank
        confirm "delete every tunnel on this server?" || return 0
        for n in $(cfg_list); do tunnel_remove "$n"; done
...
full_uninstall() {
    ...
    blank
    # Stopped before its pieces are taken away. Deleting the device under a
    # running core lets systemd restart it, and the pre-start puts the
    # device, the listener and the offload setting back - after which the
    # stop below leaves all three behind.
    for n in $(cfg_list); do
        ...
        grefou_down "$n" >/dev/null 2>&1 || true
    done
...
show_nat() {
    ...
    if [ "$any" = 0 ]; then
        ...
        dim "a Fake TCP tunnel runs, or when something under Blocking is on."
    fi
}
