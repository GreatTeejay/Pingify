...
# tunnel_port_of FILE - "port family", one line for each port a tunnel's file
# names, on the side that waits for it: its own, and one per failover backup,
# since that side listens on every one of them at once. AmneziaWG's is the
# one somebody has to open.
tunnel_port_of() {
    local f=$1 t p fam b
...
tunnel_port_owner() {
    local want=$1 fam except=${3:-} f name ports
    fam=$(port_family "$2")
    ...
    cfg_files | while read -r f; do
        name=$(cfg_name "$f")
        [ -n "$except" ] && [ "$name" = "$except" ] && continue
        # Read in full first. grep -q piped straight from tunnel_port_of stops
        # at the first match, the writer takes SIGPIPE, and under pipefail
        # the match reads as a failure - so the port listed first was never
        # the one found.
        ports=$(tunnel_port_of "$f") || continue
        grep -qx "$want $fam" <<<"$ports" || continue
        printf '%s' "$name"
        break
    done
    return 0
...
show_taken_tunnel_ports() {
    ...
    listing=$(
        cfg_files | while read -r f; do
            name=$(cfg_name "$f")
            [ -n "$except" ] && [ "$name" = "$except" ] && continue
            tunnel_port_of "$f" | while read -r port fam; do
                printf '%s %s %s\n' "$name" "$port" "$fam"
            done
        done
    )
...
show_taken_ports() {
    ...
    while IFS= read -r n; do
        ...
        any=1
        printf '%s\n' $list | dim_wrap "$(pad_to "$n" 22)${BX_ARR} " " "
    done < <(cfg_list)
    [ "$any" = 1 ] && blank
    return 0
}
