...
block_carrier_443() {
    ...
    while IFS= read -r n; do
        f=$(cfg_file "$n")
        # Every port the tunnel uses, backups included, with its family.
        if tunnel_port_of "$f" 2>/dev/null | grep -q "^443 udp$"; then
            printf '%s' "$n"
            return 0
        fi
    done < <(cfg_list)
...
block_carrier_accept() {
    ...
    while IFS= read -r n; do
        ...
        # Every port the tunnel can carry on, its failover backups included:
        # a backup the blocking rules catch is a backup that fails the day it
        # is needed.
        while read -r p fam; do
