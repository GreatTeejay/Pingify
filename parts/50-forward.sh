...
forwards_of() {
    ...
    # A forward tunnel made before the list was kept here has it in its
    # file, one token an entry, the same shape as the state.
    f=$(cfg_file "$1")
    [ -f "$f" ] && [ "$(toml_get "$f" tunnel mode)" = forward ] || return 0
    toml_arr "$f" forward ports | tr ' ' '\n' | grep . 2>/dev/null
    return 0
}
...
# fwd_owns SPECS PROTO PORT - whether a list of forward specs covers that
# port for that protocol.
fwd_owns() {
    local specs=$1 want=$2 port=$3 op olo ohi
    while read -r op olo ohi _ _; do
        [ -n "$op" ] || continue
        [ "$op" = "$want" ] || continue
        fwd_overlap "$port" "$port" "$olo" "$ohi" && return 0
    done <<<"$specs"
    return 1
}
...
forwards_clash() {
    ...
    # What this tunnel already forwards. A forward tunnel's core binds its
    # ports itself, so every one of them is "already open here" - and the
    # check read that as someone else's: adding a port to a tunnel that was
    # running refused the ports it already had, and the only way through was
    # to stop the tunnel first. A socket this tunnel holds on a port it is
    # keeping is not a clash. Anything else on that port still is, and so is
    # another tunnel's list, which is checked above from the files.
    local own=
    [ -n "$name" ] && own=$(forward_specs "$(forwards_of "$name")" 2>/dev/null)
    ...
    while read -r proto lo hi dsth dstp; do
        ...
        while read -r op oport who; do
            ...
            if [ "$who" = pingify-core ] && fwd_owns "$own" "$op" "$oport"; then
                continue
            fi
...
nat_rebuild() {
    ...
    while read -r t; do
        ...
        # A forward tunnel's ports are the core's own sockets, not DNAT.
        [ "$(toml_get "$(cfg_file "$t")" tunnel mode)" = forward ] && continue
        if [ -z "$(forwards_of "$t")" ]; then
            dev=$(tun_dev_of "$t")
            [ -n "$dev" ] && nat_forward_hook del "$dev"
            continue
        fi
...
nat_apply_all() {
    ...
    for t in $(cfg_list); do
        f=$(cfg_file "$t")
        [ "$(toml_get "$f" tunnel side)" = iran ] || continue
        [ "$(toml_get "$f" tunnel mode)" = forward ] && continue
        [ -s "$(fwd_file "$t")" ] && any=1
    done
    # Nothing to put in the kernel means nothing is touched: no chains, no
    # ip_forward, on a server that does not forward.
    [ "$any" = 1 ] || return 0
...
nat_drop() {
    ...
    # Only an IRAN private link has rules in the kernel to take out.
    local f
    f=$(cfg_file "$name")
    if [ -f "$f" ] && { [ "$(toml_get "$f" tunnel side)" != iran ] || [ "$(toml_get "$f" tunnel mode)" = forward ]; }; then
        return 0
    fi
...
nat_teardown() {
    ...
    # The sysctl file stays. Turning IP forwarding off here took away what
    # somebody chose under Optimize, because deleting a tunnel's last port
    # is not a decision about the whole machine.
    return 0
}
...
screen_ports() {
    ...
    while :; do
        ...
        2) if [ "$(toml_get "$f" tunnel mode)" = forward ]; then
                blank
                warn "a forward tunnel is its ports - to stop it, stop or delete the tunnel"
                pause
            else
                confirm "stop forwarding every port for $name?" n && nat_drop "$name"
            fi ;;
        0 | '') return 0 ;;
