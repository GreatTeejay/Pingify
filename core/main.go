...
// runOnAtLeastTwo gives the scheduler two processors on a machine that has
// one.
//
...
// ten milliseconds. Two of those is twenty, and twenty is exactly what was
// measured on a single-core server in Iran carrying sixteen carriers: the
// packet the private link had just read off the device sat there, already
// built, waiting for a turn to be handed to the wire.
//
//	                   tun -> wire        round trip
...
//
// The round trip on the wire underneath was 81, so the whole of that gap was
// this and none of it was the path. Throughput did not pay for the fix:
// sixteen streams carried 391.9 Mbit/s on one processor and 391.5 on two.
//
