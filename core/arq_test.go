...
func newLossyPair(loss, dupe float64, jitterMs int) (*arqConn, *arqConn, *lossyLink) {
	...
	a := newARQ(0x1234, 0, psk, 1200, func(b []byte) error {
		l.mu.Lock()
		l.sentAB++
		l.mu.Unlock()
		l.deliver(&l.b, b)
	...
	b := newARQ(0x1234, 0, psk, 1200, func(buf []byte) error {
		l.mu.Lock()
		l.sentBA++
		l.mu.Unlock()
		l.deliver(&l.a, buf)
...
func TestARQHeaderIsMaskedAndVaries(t *testing.T) {
	...
	var mu sync.Mutex
	psk := []byte("shared key for the arq layer only")
	c := newARQ(0xAABBCCDD, 0, psk, 1200, func(b []byte) error {
		mu.Lock()
...
func TestARQRetransmitsAndGivesUp(t *testing.T) {
	...
	sent := 0
	var mu sync.Mutex
	c := newARQ(1, 0, []byte("k"), 1200, func(b []byte) error {
		mu.Lock()
		sent++
		mu.Unlock()
		return nil
