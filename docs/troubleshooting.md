...
**از اینجا شروع کنید:** `sudo pingify` ← **Health Check**، روی **هر دو سرور**. نصف خرابی‌ها فقط از آن طرف معنی می‌دهند.

**اولین چیزی که باید رد شود: هر دو سرور یک نسخه باشند.** علامت کلاسیکش این است:

```
state    UP  -  20 of 8 carriers
```

بیست بالا در برابر کانفیگی که هشت می‌خواهد، یعنی طرف مقابل بیست تا می‌زند — چون جدول پریستش چیز دیگری می‌گوید. هر دو را آپدیت و restart کنید.

**موارد رایج:**

- **Carrier بالاست ولی Backend خطا می‌دهد** — تانل کارش را کرده؛ سرویس روی آدرس و پورت مقصد در خارج گوش نمی‌دهد
- **`no carrier up`** — خط‌های **اطراف** آن را بخوانید، آنجا دلیل افتادن کریر است
- **قطعی‌های دوره‌ای** — به ترتیب: نسخهٔ دو طرف، فایروال، Idle Timeout در NAT/CDN، اختلاف ساعت، MTU
- **قوانین Forward کهنه** — `pingify --apply-firewall`. یک Redirect قدیمی هر بستهٔ آن پورت را می‌بلعد و دقیقاً شبیه تانل خراب است

**status endpoint** بدون هیچ لاگ اضافه‌ای جواب می‌دهد: `curl -s http://127.0.0.1:9702/status`. مقایسهٔ `wire_tx_bytes` یک طرف با `wire_rx_bytes` طرف دیگر همان اندازه‌گیری‌ای است که «تقصیر تانل است یا مسیر» را تمام می‌کند.

سطح لاگ را برای عیب‌یابی روی `debug` بگذارید و **بعدش به `info` برگردانید**.
