...
نُه ترنسپورت در دو گروه:

**FORWARDING** (پورت‌های شما روی یک اتصال، هسته تنها چیز در مسیر): TCP MUX، TCP PCK، KCP FEC، UDP ARQ، WS MUX، WSS MUX

**TUN** (یک لینک خصوصی که کرنل رویش route می‌کند): ICMP، GRE، AmneziaWG

**ترتیب امتحان کردن:**

- با **TCP MUX** شروع کنید — چیزی لازم ندارد و روی اکثر مسیرها کار می‌کند. اکثر تانل‌ها باید همین‌جا بمانند.
- روی لینک **پرافت** کند بود ← **KCP FEC**
- وصل می‌شود و بعد **بی‌دلیل** می‌میرد ← **TCP PCK**
- فقط ترافیک **وب** عبور می‌کند ← **WSS MUX** با دامنه (و WS MUX روی پورت ۸۰ فقط وقتی TLS نیست)
- TCP و UDP بسته‌اند ولی **پینگ** جواب می‌دهد ← **ICMP** (کندترین، راه آخر)

**Carrier بیشتر یعنی سریع‌تر نیست.** تعداد Carrier یعنی «تانل هم‌زمان از چند جا بریده شود و باز کار کند»، نه پهنای باند.

هر دو سرور را در **همان ساعت** تست کنید و Throughput و Loss و Jitter را مقایسه کنید، نه فقط پینگ بیکار.
